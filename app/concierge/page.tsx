...
export default function Page() {
  ...
  const send = (patch: Partial<Answers>) => {
    ...
    // מתקדמים שלב-שלב עד דירוג מינימלי; את ה"Done" נעשה בלחצן
      ...
      <main className="max-w-md mx-auto px-4">
        ...
        <section className="pb-28">
          <div className="text-center text-xs text-zinc-500 mb-3">
            Tip: זה רק היכרות בסיסית. תמיד אפשר להוסיף פרטים בהמשך.
          </div>
          ...
          <div className="space-y-4">
            ...
            {step >= Q.Atmosphere && (
              <div className="flex gap-2">
                <div className="shrink-0 w-8 h-8 rounded-full bg-violet-100" />
                <div className="bg-white border rounded-2xl px-3 py-2 shadow-sm">אווירה ורעש?</div>
              </div>
            )}
            {step === Q.Atmosphere && (
              <div className="flex justify-end">
            ...
            {step >= Q.BarOrCafe && (
              <div className="flex gap-2">
                <div className="shrink-0 w-8 h-8 rounded-full bg-violet-100" />
                <div className="bg-white border rounded-2xl px-3 py-2 shadow-sm">בר/קפה?</div>
              </div>
            )}
            {step === Q.BarOrCafe && (
              <div className="flex justify-end">
            ...
            {step >= Q.MinRating && (
              <div className="flex gap-2">
                <div className="shrink-0 w-8 h-8 rounded-full bg-violet-100" />
                <div className="bg-white border rounded-2xl px-3 py-2 shadow-sm">דירוג מינימלי? (מומלץ 4+)</div>
              </div>
            )}
            {step === Q.MinRating && (
              <div className="flex justify-end">
        ...
        <div className="mt-2 flex gap-2">
          {r.price_per_person_min != null && r.price_per_person_max != null && (
            <span className="text-xs bg-zinc-100 px-2 py-1 rounded-full">
              ₪{r.price_per_person_min}-{r.price_per_person_max}
            </span>
          )}
          {r.kosher_status && r.kosher_status !== "none" && (
            <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">כשר</span>
          )}
        </div>
      </div>
    </a>
